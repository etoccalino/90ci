...
- `crates/wasm/tests/boundary.rs` — WASM boundary harness (`wasm-bindgen-test`, `run_in_browser`): a round-trip test that calls `simulate` over the built glue and asserts a plausible `SimOutput`, plus a divide-by-zero test asserting that a degenerate model returns a readable error string (never a wasm `"unreachable"` trap; E-07). Stage 3 adds one test per §4 validation row (E-05a, E-05b, E-12, E-02, E-03, E-01) asserting the thrown JS string names the offending token/row and carries the corrective phrasing; a header comment documents the one-violation-per-run precedence. Run with `wasm-pack test --headless --chrome crates/wasm`.
