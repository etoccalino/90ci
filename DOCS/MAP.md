...
- `web/package.json` scripts — `build:wasm` (invokes the script above), `dev` (`build:wasm` then `vite`), `build` (`build:wasm` then `vite build`), `preview` (Vite preview), `test` (`vitest run`), `typecheck` (`tsc --noEmit` — the dedicated no-TS-errors gate, since `vite build` does not typecheck), `e2e` (`playwright test` — the release smoke; requires a prior `pnpm build`).
