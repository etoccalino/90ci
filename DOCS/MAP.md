...
- `web/src/components/A11y.test.tsx` — Stage 6 §5/§7 tests: the distribution-semantics tooltip button is present and its honest copy reachable on both a `normal` and a `uniform` row; a copy review asserts `DISTRIBUTION_TOOLTIP` makes no percentile claim for uniform/range; every input (title, formula, per-row name/distribution/5th/95th) has an accessible name; the Run button is keyboard-focusable and activatable by Enter and Space.
